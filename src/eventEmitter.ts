...
type MqttData<Event extends MqttEvent> = Event extends 'energyTodayData' ? number : 'ON' | 'OFF';

export function waitForEventEmitterData<Events extends MqttEvent[]>(events: [...Events]): Promise<{ [K in keyof Events]: MqttData<Events[K]> }> {
    const promises = events.map((event) => {
        return new Promise<MqttData<typeof event>>((resolve, reject) => {
            ...
            setTimeout(() => reject(new Error(`Timeout waiting for ${event} MQTT data`)), 10000);
        });
    });
